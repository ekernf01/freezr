...
), which can be especially daunting for newcomers.
...
Fortunately, much of this extra work can be automated. Keep reading!

###`freeze` helps record analyses for later
 
First, some preliminaries: to install and load, run this.
...
This will create some fake code that you can pretend you've written.

```{r}
fake_proj = file.path( getwd(), "fake_project" )
dir.create(fake_proj)
cat("my_fun = function(x) plot(x)", file = file.path( fake_proj, "my_script.R" ) )
...
In this line of code, `freeze` will:
